...
L1_PROBE_URL=http://localhost:$(docker port l1_node | grep '0.0.0.0' | awk -F ':' 'NR==1 {print $2}')
...
export L1_NODE_HTTP_ENDPOINT=http://localhost:$(docker port l1_node | grep "0.0.0.0" | awk -F ':' 'NR==1 {print $2}')
export L1_NODE_WS_ENDPOINT=ws://localhost:$(docker port l1_node | grep "0.0.0.0" | awk -F ':' 'NR==2 {print $2}')
