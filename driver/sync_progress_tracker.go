...
var (
	syncProgessFetchInterval = 10 * time.Second
)
...
func NewBeaconSyncProgressTracker(c *ethclient.Client, timeout time.Duration) *BeaconSyncProgressTracker {
	return &BeaconSyncProgressTracker{client: c, ticker: time.NewTicker(syncProgessFetchInterval)}
}
...
// ResetMeta resets the inner beacon sync meta data.
func (s *BeaconSyncProgressTracker) ResetMeta() {
	s.mutex.Lock()
	defer s.mutex.Unlock()
