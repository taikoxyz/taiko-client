...
// ProgressTracker is responsible for tracking the L2 execution engine's sync progress, after
...
// NewProgressTracker creates a new BeaconSyncProgressTracker instance.
func NewProgressTracker(c *ethclient.Client, timeout time.Duration) *ProgressTracker {
	return &ProgressTracker{client: c, timeout: timeout, ticker: time.NewTicker(progressCheckInterval)}
}

// Track starts the inner event loop, to monitor the sync progress.
func (t *ProgressTracker) Track(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
...
// track tries to track the L2 execution engine's beacon sync progress.
func (t *ProgressTracker) track(ctx context.Context) {
	t.mutex.Lock()
...
func (t *ProgressTracker) UpdateMeta(id, height *big.Int, blockHash common.Hash) {
	t.mutex.Lock()
	defer t.mutex.Unlock()
...
func (t *ProgressTracker) ClearMeta() {
	t.mutex.Lock()
	defer t.mutex.Unlock()
...
func (t *ProgressTracker) HeadChanged(newID *big.Int) bool {
	t.mutex.RLock()
...
func (t *ProgressTracker) OutOfSync() bool {
	t.mutex.RLock()
	defer t.mutex.RUnlock()
...
func (t *ProgressTracker) Triggered() bool {
	t.mutex.RLock()
	defer t.mutex.RUnlock()
...
func (t *ProgressTracker) LastSyncedVerifiedBlockID() *big.Int {
	t.mutex.RLock()
	defer t.mutex.RUnlock()
...
func (t *ProgressTracker) LastSyncedVerifiedBlockHeight() *big.Int {
	t.mutex.RLock()
	defer t.mutex.RUnlock()
...
func (t *ProgressTracker) LastSyncedVerifiedBlockHash() common.Hash {
	t.mutex.RLock()
	defer t.mutex.RUnlock()
...
func syncProgressed(last, new *ethereum.SyncProgress) bool {
	if last == nil {
		return false
	}
