...
func (p *Prover) initL1Current() error {
	l1State, err := p.rpc.GetL1State(nil)
	if err != nil {
		return err
	}

	if l1State.LatestVerifiedId == 0 {
		p.l1Current = 0
		return nil
	}

	latestVerifiedHeaderL1Origin, err := p.rpc.L2.L1OriginByID(p.ctx, new(big.Int).SetUint64(l1State.LatestVerifiedId))
	if err != nil {
		return err
	}
...
func (p *Prover) isBlockVerified(id *big.Int) (bool, error) {
	...
	return id.Uint64() <= l1State.LatestVerifiedId, nil
}
