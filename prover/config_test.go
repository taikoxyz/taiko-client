...
var testFlags = []cli.Flag{
	...
	&cli.StringFlag{Name: flags.MinimumBidAmount.Name},
}
...
func (s *ProverTestSuite) TestNewConfigFromCliContext_BidStrategy() {
	l1WsEndpoint := os.Getenv("L1_NODE_WS_ENDPOINT")
	l1HttpEndpoint := os.Getenv("L1_NODE_HTTP_ENDPOINT")
	l2WsEndpoint := os.Getenv("L2_EXECUTION_ENGINE_WS_ENDPOINT")
	l2HttpEndpoint := os.Getenv("L2_EXECUTION_ENGINE_HTTP_ENDPOINT")
	...
	app.Action = func(ctx *cli.Context) error {
		...
		s.True(c.Dummy)
		s.True(c.OracleProver)
		s.Equal(c.BidStrategyOption, bid.BidStrategyMinimumAmount)
		s.Equal(c.MinimumAmount, new(big.Int).SetUint64(100000000000000))
		s.Equal(
			crypto.PubkeyToAddress(s.p.cfg.OracleProverPrivateKey.PublicKey),
			crypto.PubkeyToAddress(c.OracleProverPrivateKey.PublicKey),
		)
	...
	s.Nil(app.Run([]string{
		...
		"-" + flags.OracleProverPrivateKey.Name, os.Getenv("L1_PROVER_PRIVATE_KEY"),
		"-" + flags.Graffiti.Name, "",
		"-" + flags.BidStrategy.Name, string(bid.BidStrategyMinimumAmount),
		"-" + flags.MinimumBidAmount.Name, "100000000000000",
	}))
}
